...
3. **Кожен `check.py` має щонайменше одну перевірку на режим відмови** (префікс
   `FAILURE ·` у docstring). Щасливий шлях не доводить нічого.
4. **Усе має працювати офлайн і без API-ключа.** Якщо перевірка потребує мережі —
   вона зламана, а не мережа.
5. **Усе, що йде в репозиторій, — англійською.** Проза, docstrings, коментарі,
   повідомлення перевірок, комміти. Див. таблицю мов у CONVENTIONS.md. Чернетки й
   `sources/` — будь-якою: вони в `.gitignore`.
