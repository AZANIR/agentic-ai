...
python -m stages.s08_eval.check   # 30 перевірок, з них 14 на режими відмови
...
`46 із 110`. Два етапи групують по-різному, і **обидва праві**: етап 1 пише один `trace_run`
...
`35 із 110`. Записані файли трейсів переживуть зміну формату **мовчки**, і етап оцінюватиме
...
`44 із 110`. Кожен вердикт несе **вид оцінювача** поруч: детермінований або судить. Це не
...
оцінювачів, що судять. Двадцять один виклик на двадцять один кейс.

### `judge.py` — два протоколи, не один

`89 із 110`. Попарний і поточковий міряють різне й потрібні різному:
...
`55 із 110`. Жоден крок оцінювання не стоїть між запитом і відповіддю: усе читається з трейсу
...
Етапи позначають «який це прогін» **чотирма різними** полями — `scenario`, `phase`, `scene`,
`trace_ref`, — а два не позначають ніяк. На трейсі сервісу оцінювач сліпий на два виміри:
...
Одинадцять мутацій у [`exercises.md`](exercises.md). Найдорожчі стосуються не оцінювання, а
...
| Перевірок / на режими відмови | 30 / 14 |
| Мутацій у вправах | 11 |
| Викликів судді за прогін | 21 |
