...
# Імʼя кейса нежадібне, а не «будь-що крім `|`»: `render` екранує `|` всередині імені, і
# `[^|]+?` на такому рядку просто не спрацьовував би — читач, що додав кейс із трубою в
# назві, отримував би червону AC-01b про розбіжність лічильників, тобто не про свою причину.
# Розділювач — саме « | » з пробілами по краях, а екранований `\|` їх не має.
...
def render(report: Report) -> str:
    ...
    for row in report.rows:
        ...
        # Імʼя кейса екранується: `|` всередині нього розірвав би рядок таблиці, `parse()`
        # недорахував би, і читач, що додав такий кейс, отримав би червону AC-01b з
        # повідомленням про розбіжність лічильників — червоне не про свою причину.
        out.append(f"| {row.case.name.replace('|', chr(92) + '|')} | " + " | ".join(cells) + " |")
