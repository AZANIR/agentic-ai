...
SHARED_SCRIPT = (
    "   Сценарій моделі спільний для всіх реалізацій: інакше токени були б неспівмірні",
    "   ще до того, як фреймворк щось додав. Висновки — про співвідношення, а не про",
    "   абсолютні числа.",
)
...
RULES = [
    ...
    ("задача розгалужується", "виміряй у себе: ця таблиця про два кроки", "мої рядки"),
]
...
def collect(traces: Path) -> list[compare.Row]:
    ...
    for module, name, packages in IMPLEMENTATIONS:
        title = module.NAME
        # Порядок такий самий, як у §6 потік 3 SAD: спершу ПРАПОРЕЦЬ, потім доступність.
        # Зворотний порядок радив поставити пакет тому, хто його не просив, — і робив
        # задокументований рядок «прапорець вимкнено» недосяжним саме на тій установці,
        # для якої його писали.
...
def scene_tokens(rows: list[compare.Row]) -> None:
    ...
    print()
    print("   Порівняй із четвертою сценою. Оркестратор бере РЯДКАМИ й додає нуль токенів —")
    print("   і це виміряно. Що фреймворк ролей узяв би навпаки — очікування, а не вимір:")
    print("   його рядок у таблиці порожній, і етап не має права видавати одне за інше.")
...
def main(*, table_path: Path | None = None) -> int:
    # Гучно й ДО таблиці — але з демо, а не зі збірки: `collect()` кличуть і перевірки.
    via_adk.demand()

    client = _client()
    # Банер із `shared.llm`, а не літерал. Літерал казав «модель підроблена» й тоді, коли
    # читач налаштував ключ, — тобто брехав саме тому, хто зробив усе правильно.
    print(banner(client))
    for line in SHARED_SCRIPT:
        print(line)
    print()
