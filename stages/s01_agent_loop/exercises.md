...
У [`loop.py`](loop.py) знайди рядок із `tool.irreversible` і зроби так, щоб гейт не спрацьовував.
...
- Перевірка червоніє. **Рівно одна** — та, що стверджує «незворотна функція не викликається».
- Перевірки підтвердженого шляху й зворотних інструментів лишаються зеленими.

**Питання, на яке треба вміти відповісти:** чому впала саме одна перевірка, а не три? Що це
каже про те, наскільки точно тест описує поведінку?
