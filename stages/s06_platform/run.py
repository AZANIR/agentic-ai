...
Працює **без API-ключа, без мережі й без контейнерів**: сцени викликають `Service` напряму,
а провайдер — підробка. HTTP-поверхня перевіряється окремо (`check.py`), бо вона потребує
встановленого веб-фреймворка, а демо має працювати на базовій установці. Перший рядок
виводу каже, що саме працює.
...
def scene_mirrors(tmp: Path, tracer) -> None:
    ...
    print("  смоук:            третій стан є в самому скрипті — див. нижче")
    print()
    ...
    print()
    print("  Числа смоуку тут НЕ друкуються навмисно: демо його не запускає, а число,")
    print("  взяте з памʼяті автора, — це рівно та проза, що розходиться з кодом.")
    print("  Запусти сам:  API_KEY=<ключ> ./deploy/smoke.sh https://localhost")
    print()
