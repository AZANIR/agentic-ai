...
- **Статей у репозиторії немає взагалі.** Вони пишуться прямо в репозиторій блога, і там же
  лежать їхні `claims.json`. Звірка з кодом автоматизована: `scripts/article_check.py` читає
  блог і перераховує кожне твердження на теґу, який стаття називає.
