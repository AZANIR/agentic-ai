...
| C-7 | Стан лімітів і бюджету **не в пам'яті процесу** | інакше етап містить ваду, якої навчає |
...
C4Context
    ...
    System_Ext(pg, "Postgres", "Памʼять і трейси переживають перезапуск")
    ...
    Rel(api, pg, "Читає й пише памʼять і трейси")
...
воркерів і її виправлення, сховище для пам'яті й трейсів, TLS через зворотний проксі, скрипт
...
| Стан лічильників | Redis, не пам'ять процесу | Інакше етап містить ваду, якої навчає. ADR-0002 |
...
├── run.py          демо: сцени проти TestClient, без мережі
├── check.py        перевірки
...
C4Container
    ...
    Rel(app, mem, "Читає й пише факти")
    Rel(mem, pg, "Коли профіль prod")
...
```mermaid
C4Container
    title Розгортання — одна машина, чотири контейнери
    ...
    Container_Boundary(vm, "Одна машина") {
        ...
        Container(pg, "Postgres", "pgvector", "Памʼять, трейси")
...
**Дані живуть у томах**, а не в контейнерах: перезапуск сервісу не стирає ні пам'яті, ні
трейсів (AC-10).
...
| Аспект | Як вирішено |
|---|---|
| Трейс | Один запит — один трейс; кожен крок несе причину. Кроки етапів 2 і 5 відсутні свідомо (ADR-0005) |
