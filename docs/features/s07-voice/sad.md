...
| Розклад | Кроки **плюс віддача споживачеві**, залишок = нуль | Дві третини інваріанта лишали час споживача на рахунку моделі. ADR-0007 |
| Трейс | Трасувальник — необовʼязковий параметр | Перевірки лишаються офлайновими, а звірка двох механізмів стає можливою. ADR-0008 |
...
C4Container
    ...
    Container_Boundary(s07, "stages/s07_voice") {
        ...
        Container(model, "model.py", "Python", "Підроблена модель і розкид затримок")
        Container(ws, "ws.py", "Python", "Сокет живого режиму")
        Container(real, "real.py", "Python", "Справжні адаптери — написані, не прогнані")
    ...
    Rel(pipe, model, "Відповісти")
    Rel(pipe, tts, "Озвучити")
    Rel(stt, real, "За прапорцем")
    Rel(tts, real, "За прапорцем")
...
**Чому `shared/llm.py` тут немає.** Етап міряє **конвеєр**, а не модель. Справжній виклик
додав би мережу, ключ і розкид, якого не можна відтворити, — тобто зробив би головне
число невимірюваним. «Модель» тут — `model.py`: підроблена затримка, чия величина є
функцією номера прогону. Перша редакція діаграми малювала `Rel(pipe, llm)`, і це було
неправдою: конвеєр не кликав `shared/llm.py` жодного разу.
...
| 0007 | Розклад має три доданки, а не два | Accepted | §4, §6, §10 |
| 0008 | Трасувальник — необовʼязковий параметр конвеєра | Accepted | §4, §5 |
...
| **Реального мікрофона немає** | High | AC-07 лишається `НЕ ПЕРЕВІРЕНО`. Сокет тепер **запускається** перевіркою (AC-07c) — без звуку, але з повною розмовою; неперевіреним лишається саме звук. Не приховується | Learner, перед тегом |
| **`real.py` не прогнано** | High | Написано, не виконано: ваг моделей і мікрофона в автора немає. Найслабше місце етапу, назване в обох README | Learner, перед тегом |
