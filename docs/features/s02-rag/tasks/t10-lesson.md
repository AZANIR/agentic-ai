...
`README.md` (UA): що зможеш → канон (embed → cosine → top-k) → міст на NovaShop → три межі, названі прямо. Обов'язково: розрив на синонімах як **очікувана** поведінка, а не поламка; чому джерело додає система; чому фільтр стоїть до відбору. `README.md` — один екран.
