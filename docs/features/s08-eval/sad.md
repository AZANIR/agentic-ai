...
| Детермінований чек проти судді | Кожен **оцінювач** оголошує свій вид, і це перевіряється | Суддя за порівняння рядків — це витрата й ненадійність одночасно. ADR-0004 |
...
**Потік 2b — детектор length bias, поточковий протокол (AC-06).**
...
sequenceDiagram
    ...
    B->>J: оціни коротку правильну відповідь
    J-->>B: бал
    B->>J: оціни ТУ САМУ плюс правдивий зайвий текст
    J-->>B: бал
    B->>B: різниця балів - числом, без порога
    B-->>B: ЗНАХІДКА: length bias, якщо різниця строго додатна
    Note over B,J: Порога тут немає й бути не може: зміст той самий, додано лише символи
```

Протоколів два, і це окреме рішення етапу: у попарному немає **бала**, у поточковому немає
**порядку**. Один протокол не показав би обох біасів.
...
| 0004 | Кожен оцінювач оголошує, детермінований він чи судить | Accepted | §4, §8 |
...
| Режими відмови | Перелік перевірок | ≥ 1/3 із префіксом `FAILURE` | підрахунок, NFR-4 |
...
| Оцінювач сам стає нетривіальним і неперевіреним | Medium | Чотирнадцять мутацій ламають властивості; червоніє перевірка, що стверджує саме про неї. Незалежне рев'ю знайшло вісім MAJOR — саме тому мутації є, а не лише зелений набір | Contributor, перед тегом |
