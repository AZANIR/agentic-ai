...
```
stages/s04_mcp/
├── server.py       FastMCP-сервер: оголошує інструменти етапів 1–2; ≤60 рядків
...
C4Container
    ...
    Container_Boundary(proc, "окремий процес") {
        Container(server, "server.py", "Python + FastMCP", "Оголошує й виконує")
    }
...
|---|---|---|---|
| **Перевірки з підпроцесом повільні й мигтливі** | High | Кожен тест піднімає власний сервер із тайм-аутом; NFR-5 підняв межу до 8 с свідомо. Досвід етапів 1–3: приховати ціну гірше, ніж назвати | Contributor |
...
| **API бібліотеки MCP зміниться** | Medium | Extra з підлогою, не пін; перевірка `list_tools` впаде голосно. Стаття-джерело описує зміну протоколу як свіжу новину — це прямо той випадок | Contributor |
