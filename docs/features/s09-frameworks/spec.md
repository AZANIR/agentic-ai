...
**Given** фреймворк, що додає власний текст до кожного запиту
**When** харнес рахує токени понад запит
**Then** це число **строго додатне** щонайменше для однієї фреймворкової реалізації, і
дзеркальна половина обов'язкова: для базової лінії воно **дорівнює нулю**.

Лічильник, що показує надбавку скрізь, не відрізняє фреймворк від власного коду й тому
лічильником не є
...
| AC-04b | `the overhead is positive for a framework and zero for the baseline` | integration | Дзеркальна половина: лічильник розрізняє |
...
- **Що реалізація ADK працює.** Вона написана, але автор не має креденшелів. Її рядок лишається
  `НЕ ПЕРЕВІРЕНО`, і це найслабше місце етапу — назване, а не сховане (§11 SAD).
...
| NFR-8 | Версії фреймворків **запінені** мінорною межею | зміна мажорної версії ламає смоук, а не прогін читача | звірка `pyproject.toml` у перевірці |
