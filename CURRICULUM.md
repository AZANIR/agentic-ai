...
**Статус на 2026-08-23:** скелет матеріалізовано, **етап 1 завершено**, етапи 2–10 не почато.
...
до інструмента.
**Залежності:** —
**Готово:** 21 перевірка (8 на режими відмови), демо 0.12 с, перевірки 1.05 с,
`loop.py` 116/120 рядків, `validate.py` 42/60. [Урок](stages/s01_agent_loop/README.md)
...
**Будуєш:** FastMCP-сервер і stdio-клієнт; агент етапу 3 переходить із локальних функцій
...
**Доказ:** парсер відповіді ігнорує narration-блоки і робить `json.loads` на
`mcp_tool_result` — точна пастка, описана у статті 6.
**Залежності:** 3
...
не потрапляє. Друга половина важливіша за першу.
**Залежності:** 2
